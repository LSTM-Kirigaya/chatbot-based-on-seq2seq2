...
class EncoderRNN(nn.Module):
    ...
    def forward(self, input_seq, input_length, hidden=None):
        ...
        embedded = self.embedding(
            input_seq
        )  # embedded:[batch_size, max_length, embedding_dim], 此处embedding_dim==hidden_size
...
class LuongAttentionLayer(nn.Module):
    ...
    def __init__(self, score_name, hidden_size):
        ...
        if score_name not in ["dot", "general", "concat"]:
            raise ValueError(
                self.score_name,
                "is not an appropriate attention score_name(dot, general, concat)"
            )
        # general有一个向量的外积变换， concat有一个外积变换与一个内积变换，所以这两个方法会产生额外的网络参数
        # 下面为这两个方法设置需要的网络参数
        if score_name == "general":
            self.Wa = nn.Linear(hidden_size,
                                hidden_size)  # 使用h->h的全连接层模拟h*h的矩阵与h维列向量的乘法
        elif score_name == "concat":
            self.Wa = nn.Linear(
                hidden_size * 2,
                hidden_size)  # 需要将h_t与h_s联级再做矩阵乘法，所以是2h->h的全连接层
            self.v = nn.Parameter(
                torch.FloatTensor(hidden_size)
            )  # FloatTensor会生成hidden_size维的全零（浮点0）向量，我们通过参数可更新的向量来模拟内积变换

        # 计算attention vector的变换
        self.Wc = nn.Linear(hidden_size * 2,
                            hidden_size)  # 需要将c_t与h_t联级再做矩阵乘法，所以是2h->h的全连接层
    ...
    def concat_score(self, current_hidden, encoder_output):
        concat = torch.cat(
            [current_hidden.expand(encoder_output.shape), encoder_output],
            dim=2)
    ...
    def forward(self, current_hidden, encoder_output):
        ...
        # 通过广播运算完成
        context_vector = torch.sum(encoder_output *
                                   attention_weights.unsqueeze(2),
                                   dim=1)
        # context_vector : [batch_size, hidden_size]
        # 有了上下文向量，就可以计算attention vector了
        attention_vector = self.Wc(
            torch.cat([context_vector.unsqueeze(1), current_hidden],
                      dim=2)).tanh()
...
class LuongAttentionDecoderRNN(nn.Module):
    ...
    def forward(self, input_single_seq, last_hidden, encoder_output):
        ...
        embedded = self.embedding(
            input_single_seq)  # [batch_size, max_length, hidden_size]
        embedded = self.embedding_dropout(
            embedded)  # [batch_size, max_length, hidden_size]
...
if __name__ == "__main__":
    ...
    output, current_hidden = decoder.forward(input_single_seq=x,
                                             last_hidden=None,
                                             encoder_output=out)
